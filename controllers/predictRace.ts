...
async function createRace() {
  try {
    ...
    var race = await PredictRaceModel.create(CAMEL_RACE[randomRaceIndex]);
...
async function fetchRaces(
  ...
  try {
    ...
    const count = await PredictRaceModel.count(query);
    races = await PredictRaceModel.find(query)
      .limit(pageSize)
      .sort({ createdAt: -1 })
...
async function fetchRaceById(id: string) {
  try {
    const race: any = await PredictRaceModel.findById(id);
  ...
  try {
    ...
    if (race && data) {
      if (data?.participation) {
        ...
        const race: any = await PredictRaceModel.remove({id:id});
